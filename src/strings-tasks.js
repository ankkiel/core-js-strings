...
function countVowels(str) {
  return str.length - str.replace(/[aei0uyAEIOUY]/g, '').length;
}
...
function encodeToRot13(str) {
  const encodedStr = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz';
  const decodedStr = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';
  let result = '';

  for (let i = 0; i < str.length; i += 1) {
    const idx = encodedStr.indexOf(str[i]);
    if (idx !== -1) {
      result += decodedStr[idx];
    } else {
      result += str[i];
    }
  }
